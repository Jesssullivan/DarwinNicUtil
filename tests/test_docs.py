...
def test_artifacts_docs_match_current_release_policy():
    ...
    assert "https://flakehub.com/f/Jesssullivan/DarwinNicUtil/v2.1.3" in artifacts
    assert "current public" in artifacts
    assert "FlakeHub releases are `v2.1.3`" in artifacts
    assert "nix run github:Jesssullivan/DarwinNicUtil -- status" in artifacts
    assert "https://flakehub.com/f/Jesssullivan/DarwinNicUtil/v2.1.3" in readme
...
def test_pypi_publish_surface_is_live_and_documented():
    ...
    assert "latest validated upload is `2.1.3`" in artifacts
...
def test_project_spec_is_public_and_generic():
    ...
    assert "PyPI trusted publishing and validated `2.1.3` upload" in spec
...
def test_root_entrypoint_uses_package_app_version():
    ...
    assert result.stdout.strip() == "darwin-nic 2.1.3"
